---
title: "セルフホスト環境で Next.js のキャッシュを活用する Custom Next.js Cache Handler"
emoji: "🔄"
...
**このような環境でビルド時キャッシュが有効なままだと、ビルドフェーズで Redis サーバーに接続できずビルドが失敗しするため、ビルド時キャッシュを無効にする必要があります。**
...
ただし、Full Route Cache だけでなく Data Cache もビルド間でキャッシュが共有されなくなるため、[Data Cache のデプロイ間でキャッシュが維持される](https://nextjs.org/docs/app/building-your-application/caching#invalidation)という特性が失われてしまうことに注意しましょう。
