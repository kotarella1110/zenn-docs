...
そこで、Next.js App Router アプリケーションの実運用化に向けた PoC を SRE の [@shmokmt](https://x.com/shmokmt) さんと共に実施しました。
...
一方で、Redis サーバーにアクセス可能なデプロイ環境では、上の条件式を削除することでビルド時キャッシュが有効になります。ただし、注意点として、ビルド時にキャッシュされるのは Data Cache のみで、Full Route Cache は キャッシュされず Redis に保存されません。これについては[「ビルドプロセス中の事前レンダリングが無駄になる」](#ビルドプロセス中の事前レンダリングが無駄になる)で詳細な言及をしています。
...
実装例では、`estimateExpireAge` オプションを指定して、キャッシュの期限切れ期間をキャッシュの有効期間の3倍に設定しています。^[キャッシュの有効期間である `next.revalidate` で指定した値が `exstimateExpireAge` の引数（`staleAge`）に渡されます。`estimateExpireAge` の返り値がキャッシュの期限切れ期間になります。]
...
例えば、キャッシュの有効期間 (`next.revalidate`) が 5 秒の場合は以下のような挙動となります。

| 期間                     | 設定値     | 説明                                                                                                                                                                                               |
| ------------------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| キャッシュの有効期間     | 5秒        | リクエストから5秒間キャッシュは新鮮なため再検証が行われません。                                                                                                                                    |
| キャッシュの期限切れ期間 | 5秒×3=15秒 | 5秒経過した後の「6秒〜15秒」の間に最初のリクエストがあれば `STALE` なデータが返され、、バックグラウンドでデータの再検証が行われます。15 秒を過ぎると期限切れのためキャッシュは完全に削除されます。 |
...
サーバーが終了シグナル（`SIGTERM` や `SIGINT`）を受け取った際に、安全にサーバーを終了する Graceful Shutdown の対応が必要であることを SRE の [@shmokmt](https://x.com/shmokmt) さんから教えていただきました。
