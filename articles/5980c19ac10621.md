...
Redis クライアントは生成時に Redis サーバーとのコネクションを確立します。セルフホスト環境での Next.js アプリケーションのデプロイフローでは、ビルド時に Redis サーバーにアクセスできないケースが一般的だと思います。例えば、PoC では、GitHub Actions を使って Next.js アプリをビルドし、その成果物で起動するサーバー用の Docker イメージを Amazon ECR にプッシュし、Amazon ECS にデプロイしています。このフローでは、当然のことながら、GitHub Actions から Redis サーバーへ直接アクセスできません。^[AWS CodeBuild を使えば、Redis を配置した VPC 内でビルドしてアクセス可能な環境を構築できそうですが、手間がかかります。]
そのため、実装例では、`process.env.NEXT_PHASE` を利用してビルドフェーズでは Redis クライアントを生成しないようにしています。これによって、ビルド時には Redis サーバーへの接続を防ぎ、キャッシュをメモリに保存する [`local-lru` ハンドラー](https://caching-tools.github.io/next-shared-cache/handlers/local-lru)をフォールバックとして利用するようにしています。
...
一方で、Redis サーバーにアクセス可能なデプロイ環境では、上の条件式を削除することでビルド時キャッシュが有効になります。ただし、注意点として、ビルド時にキャッシュされるのは Data Cache のみで、Full Route Cache は Redis に保存されません。これについては後述します。
...
先に説明した通り、Redis サーバーにアクセス可能な環境でビルド時キャッシュを有効にしても、キャッシュされるのは Data Cache のみで、Full Route Cache はキャッシュされません。この動作は、`@neshca/cache-handler` ではなく Next.js の仕様、もしくは制限によるものみたいです。
Next.js はビルドプロセス中にページを事前レンダリングし、その結果を Full Route Cache として保存しますが、この処理では CacheHandler を経由しないため、Redis に保存されません。具体的には、キャッシュは Cache Handler の `set` メソッドを通して保存されますが、ビルドプロセス中に Full Route Cache を保存する際にはこの `set` メソッド呼び出されず、ファイルシステム上に保存されます。実際に確認すると、Data Cache は `.next/cache` ディレクトリに保存されず Redis に保存されますが、Full Route Cache は `.next/server/app/**/*.{html|rsc}` に保存されていることがわかります。
その結果、デプロイ直後のリクエストでは、ビルドプロセス中の事前レンダリングで生成されたキャッシュを利用できず、レンダリングのコストが発生します。つまり、ビルドプロセス中の事前レンダリングが無駄になります。

この問題に対処するため、`@neshca/cache-handler` は `registerInitialCache` API を提供しています。優秀ですね。
この問題を対処するために、この API を [Instrumentation](https://nextjs.org/docs/app/building-your-application/optimizing/instrumentation) 経由で呼び出すことで、Next.js サーバーの起動時に、ビルドプロセス中の事前レンダリングで生成されたファイルシステム上のキャッシュを Redis に保存できます。
詳しい手順については公式ドキュメントをご参照ください。

https://caching-tools.github.io/next-shared-cache/usage/populating-cache-on-start

更に、ビルドプロセス中にファイルシステム上に生成された Data Cache についてもサーバー起動時に Redis に保存できます。そのため、この API を使用することでビルド時に Redis にアクセスできるデプロイ環境を用意する必要がなくなります。
