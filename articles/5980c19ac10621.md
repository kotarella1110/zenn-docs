...
したがって、以下のような実装が必要になると考えています（雑な例を示します）。ただし、この自前実装の場合、処理中のリクエストを完了させてから Next.js サーバーを停止する動作も含める必要があるかどうかは、まだ Next.js のコードを詳しく確認していないため不明です。
...
セルフホスト環境で Next.js のキャッシュ機能を最大限に活用するには、Custom Next.js Cache Handler の活用が必須となります。特に、`@neshca/cache-handler` を使えば、キャッシュの永続化を簡単に実現できます。しかし、これらを活用するにあたっては注意すべきポイントがいくつか存在します。
